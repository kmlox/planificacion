...
class Nivel extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_nivel, nombre_nivel', 'required'),
			array('id_nivel', 'numerical', 'integerOnly'=>true),
			array('nombre_nivel', 'length', 'max'=>45),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_nivel, nombre_nivel', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'grados' => array(self::HAS_MANY, 'Grado', 'id_nivel'),
		);
	}
